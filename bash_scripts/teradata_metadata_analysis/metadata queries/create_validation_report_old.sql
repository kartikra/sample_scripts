...
--								error 104 - changed join to HCCLOH3.CLARITY_TBL to use same logic as previously modified logic in other queries. 
...
-- 05/04/2009	Jack Richter	error 53 - removed check of count in HCCLOH3.CLARITY_TBL and removed unnecessary upper/trim commands 
--									modified for perf improvments (3:28 to 1:30 in testing) 
--								error 54 - added check of count in HCCLOH3.CLARITY_TBL 
...
--								errors 101 thru 108, 901,902 - correct join to HCCLOH3.CLARITY_TBL for changes in Compass 
...
-- 06/10/2009	Jack Richter	error 53  - chgd text HCCLOH3.CLARITY_COL to : COL_TBL 
...
--								error 304 - corrected quotes and = in subqueries to in ('HCCLDOH9_B_T')
...
--								error 505 changes :UPGRADE to HCCLOH_USHARE
-- <version emailed>
...
-- 02/21/2011	Jack Richter	error 50,51 - added checks for staging table between WITS & WITS
...
--								error 51 - changed subquery to use HCCLOH_USHARE.UPGRADE_DB_OWNER_LINK dbol (so it doesn't error all tables in all deployments).
...
--								error 2,3,4,5,6-9 - chgd from 'HCCLDOH9_B_T' to HCCLDOH9_B_T
...
--
--	HCCLOH_USHARE.UPGRADE_COLUMNS			contains the column info from WITS (for wits testing) or from WITS (post upgrade testing)
--	HCCLOH_USHARE.UPGRADE_TABLES			contains the table names from WITS (for wits testing) or from WITS (post upgrade testing)
--	HCCLOH_USHARE.UPGRADE_COLUMNS_VIEWS	contains the view name & view column names from WITS (for wits testing) or from WITS (post upgrade testing)
--	HCCLOH_USHARE.UPGRADE_MANIFEST		contains the table and column change info from the upgrade manifest spreadsheet
-- 	HCCLOH_USHARE.UPGRADE_ISSUES			table should be empty - it will be populated by this script.
-- ----------------------------------------------------------------------------------------- 
-- 
--
...
-- 
-- 2) replace HCCLOH_USHARE with ushare database where upgrade_xxx tables exist - only one value may be sepecified. 
--	No quotes!.... ie. HCCLOH_USHARE 
-- 
-- 3) replace 'HCCLDOH9_B_T' with the list of reporting databases to be checked this deployment - multiple values in Califs
--		(note: in the ROCs this should be the same value placed in HCCLDOH9_B_T)
--		(note: in califs this should include the  materialized view reporting databases
--	Include quotes!.... ie. 
--					Scal Wits - 'HCCLDSC9A_RESC_T','HCCLDSC9KP_RESC_T'
--					Scal WITS - 'HCCLPSCA_T', 'HCCLPSCKP_T'
--					Ncal Wits - 'HCCLDNC9A_T', 'HCCLDNC9KP_T'
--					Ncal WITS - 'HCCLPNC2A_T','HCCLPNCKP_T'
--					region - 'HCCLDHI9_T'
--
-- 5) replace 'HCCLDOH9_B_S' with the list of staging databases to be checked this deployment - multiple values in Califs
...
--					region - 'HCCLDHI9_S'
--		
-- 6) replace HCCLDOH9_B with the database layer where the Epic views reside - only one value may be sepecified. 
--	No quotes!.... ie HCCLPHI 
-- 
-- 7) replace HCCLOH3 with the database layer where the user views reside - only one value may be sepecified. 
--	No quotes!.... ie HCCLHI 
-- 
-- 8) replace HCCLDOH9_B_T with the name of the or deployment reporting table database this validation is for.  
--	No quotes!.... ie HCCLPHI_T,  HCCLPSC9A_RESC_T, HCCLDNC9A_T, etc..
--

-- 9) replace 120180 with the physical owner of the deployment
--	No quotes! ... ie 120312
--		CO	120140
--		GA	120180
--		HI	120180
...
--		SCF	121222
--
-- 10) replace WITS with environment you are validating.  Valid values are WITS or WITS.
--	No quotes! ... ie WITS -or- WITS

-- 11)replace 0 with the error numbers to skip.  you must at least skip error 0. 
--	No Quotes!.... ie 0,901,902,903,904 
--
-- 12)replace 'HCCLDOH9_B_T' - Lead (in Califs) or regional reporting database name (in ROCs)
--	Include quotes!.... ie. 
--					Scal - 'HCCLDSC9_RESC_T'
--					Ncal - 'HCCLDNC9_T',
--					ROC  - 'HCCLDHI_T'
--
-- 13)replace 'HCCLDOH9_B_S' - Lead (in Califs) or regional staging database name (in ROCs)
--	Include quotes!.... ie. 
...
-- Note: When running in WITS, USHARE contains the structures as they exist WITS. 
--	 	 When running in WITS, USHARE contains the structures as they exist WITS. 
-- --------------------------------------- 
...
--  =====================================
--  MY_RUNNAME        runname,
--  MY_USHAREDB  ushare_db,
--  'MY_ENV'         env,
--   MY_USERDB       user_db,
--   MY_EPICDB    epic_db,
--  'MY_REPORT_DB' rpt_dbs,
--   MY_REPORT_DB  rpt_dbs_withoutquotes,
--  'MY_STAGE_DB' stg_dbs,
--  'MY_LEAD_REPORTDB' lead_rpt_db,
--  'MY_LEAD_STAGEDB' lead_stgt_db,
...
from dbc.tables t2
where t2.databasename in ('MY_REPORT_DB','MY_STAGE_DB','MY_USERDB',
...
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN TABLE_ID;
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN (COLUMN_NAME,TABLE_ID,IS_EXTRACTED_YN);
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN (TABLE_ID,IS_EXTRACTED_YN);
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN (TABLE_ID,CM_PHY_OWNER_ID);
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN (COL_DESCRIPTOR,CM_PHY_OWNER_ID);
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN (TABLE_ID,IS_EXTRACTED_YN,CM_PHY_OWNER_ID);
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN (COLUMN_NAME,TABLE_ID,CM_PHY_OWNER_ID);
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN CM_PHY_OWNER_ID;
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN IS_EXTRACTED_YN;
COLLECT STATISTICS MY_REPORT_DB.CLARITY_TBL COLUMN IS_EXTRACTED_YN;
COLLECT STATISTICS MY_REPORT_DB.CLARITY_TBL COLUMN CHRONICLES_MF;
COLLECT STATISTICS MY_REPORT_DB.CLARITY_TBL COLUMN (TABLE_NAME,EXTRACT_FILENAME,DEPENDENT_INI ,CHRONICLES_MF);
COLLECT STATISTICS MY_REPORT_DB.CLARITY_TBL COLUMN (TABLE_NAME,CM_PHY_OWNER_ID);
COLLECT STATISTICS MY_REPORT_DB.CLARITY_TBL COLUMN CM_PHY_OWNER_ID;
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN COLUMN_NAME;
COLLECT STATISTICS MY_REPORT_DB.CLARITY_COL COLUMN COL_DESCRIPTOR;
...
(0, '2. Parameters - deployment name = MY_DEPLOY_NAME', 'MY_REPORT_DB', 'MY_RUNNAME', CURRENT_TIMESTAMP(0));
INSERT INTO MY_USHAREDB.upgrade_issues (err_no, err_msg, dbname, runname, create_dttm) VALUES 
...
FROM 	MY_USHAREDB.upgrade_columns c1
		...
		AND t2.databasename in ('MY_REPORT_DB')
...
FROM 	MY_USHAREDB.UPGRADE_DBC_COLUMNS c1
		...
		AND UPPER(TRIM(c1.databasename)) in ('MY_REPORT_DB')
		-- ------------------------------------- 
		-- check if this error should be skipped 
		-- ------------------------------------- 
		AND 3 NOT IN (0)		
...
WHERE 	UPPER(TRIM(t2.databasename)) in ('MY_REPORT_DB')
...
WHERE	UPPER(TRIM(t1.databasename)) in ('MY_REPORT_DB')
...
-- --------------------------------------------------------- 
-- Check all WITS columns definitions match WITS. 
-- --------------------------------------------------------- 
...
FROM (SELECT
    ...
    FROM 	MY_USHAREDB.upgrade_columns c1
			...
			-- -------------------------------------------------------------------- 
			INNER JOIN MY_USHAREDB.UPGRADE_DBC_TABLES t2
				ON t2.databasename in ('MY_REPORT_DB')
			...
			INNER JOIN MY_USHAREDB.UPGRADE_DBC_COLUMNS c2
				...
				AND UPPER(TRIM(c2.databasename)) in ('MY_REPORT_DB')
...
FROM 	MY_USERDB.CLARITY_COL c1
		INNER JOIN MY_USERDB.CLARITY_TBL t1
			...
			AND EXISTS (
				...
				AND UPPER(TRIM(t.databasename)) IN ('MY_REPORT_DB','MY_REPORT_DB') 
		...
		-- ushare list when running validation in WITS but not in the database
		-- ---------------------------------------------------------------------------
		LEFT OUTER JOIN (
...
WHERE 	NOT EXISTS (
		...
		WHERE	t3.databasename IN ('MY_REPORT_DB','MY_REPORT_DB') 
...
FROM 	MY_USHAREDB.UPGRADE_DBC_COLUMNS c1
		...
		AND UPPER(TRIM(c1.databasename)) IN ('MY_REPORT_DB','MY_REPORT_DB') 
...
WHERE	NOT EXISTS ( 
			SELECT 1 
			FROM MY_USHAREDB.UPGRADE_DBC_TABLES t2
			WHERE t2.databasename in ('MY_REPORT_DB', 'MY_REPORT_DB')		
...
FROM (
	...
	FROM	
		...
		INNER JOIN MY_USERDB.CLARITY_TBL t1
			...
			AND c2.databasename in ('MY_REPORT_DB','MY_REPORT_DB')
...
AND NOT EXISTS ( 
			SELECT 1 
			FROM MY_USHAREDB.UPGRADE_DBC_TABLES t2
			WHERE t2.databasename IN ('MY_STAGE_DB', 'MY_STAGE_DB') 
			AND TRIM(t2.tablename) = TRIM(t1.table_name)
			AND t2.tablekind in ('T','V')
			AND t1.is_extracted_yn = 'Y'
			AND t1.cm_phy_owner_id = 'MY_OWNER_ID')
...
WHERE	c1.cm_phy_owner_id = 'MY_OWNER_ID'
		...
		AND EXISTS (
			SELECT	1
			FROM	MY_USHAREDB.UPGRADE_DBC_TABLES t2
			WHERE	t2.databasename IN ('MY_REPORT_DB')
		...
		AND NOT EXISTS (
			SELECT	1
			FROM 	MY_USHAREDB.UPGRADE_DBC_COLUMNS c2
			WHERE	c2.databasename IN ('MY_REPORT_DB')
...
FROM 	(select databasename, tablename, columnname
		...
		WHERE	databasename in ('MY_REPORT_DB')
		) as c1
		-- -----------------------------------------------------
		-- only check for table/columns valid for the deployment
		-- -----------------------------------------------------
...
AND		NOT EXISTS (
			...
			WHERE	tbl.databasename IN ('MY_REPORT_DB')
...
FROM	(SELECT	
	...
	FROM 	MY_USHAREDB.upgrade_columns c1
			...
			AND c2.columnname = c1.columnname
			AND c2.databasename in ('MY_REPORT_DB')
			AND c1.cm_phy_owner_id = 'MY_OWNER_ID'	
...
AND 	EXISTS (
			...
			WHERE	c9.databasename in ('MY_REPORT_DB')
...
AND		EXISTS (
			...
			AND 	c.databasename in ('MY_REPORT_DB') )
AND		NOT EXISTS (
			SELECT	1
...
AND		NOT EXISTS (
			...
			WHERE	c.databasename in ('MY_REPORT_DB')
...
FROM 	( 
		...
		ON c.databasename in ('MY_REPORT_DB')
		AND c.tablename = m.tablename
		AND c.columnname = m.columnname
...
and 	m.tablename not in 
	(select tablename  
	FROM	MY_USHAREDB.UPGRADE_DBC_TABLES t1
	WHERE t1.databasename IN ('MY_REPORT_DB', 'MY_REPORT_DB')
	-- ---------------------------- 
	-- exclude test and work tables 
	-- ---------------------------- 
	AND	t1.tablename NOT LIKE ALL ('BF%','UPD%','UTL%', '%/_DELETE','%/_UPDATE', '%/_DELETE_CT','%/_UPDATE_CT' ) ESCAPE '/'
...
FROM  	MY_USHAREDB.UPGRADE_DBC_TABLES t1
		...
		INNER JOIN MY_USERDB.CLARITY_TBL tbl
			...
			AND tbl.cm_phy_owner_id = 'MY_OWNER_ID'
			AND tbl.is_extracted_yn = 'Y'
			AND t1.databasename in ('MY_REPORT_DB')
			-- ------------------------------------- 
			-- check if this error should be skipped 
			-- ------------------------------------- 
			AND	101 NOT IN (0)
...
FROM	MY_USHAREDB.UPGRADE_DBC_TABLES t1
		...
		-- ushare list when running validation in WITS but will be when run in WITS.
		-- ---------------------------------------------------------------------------
		LEFT OUTER JOIN  (
			SELECT	tablename, case when chg_type = 'View Added' then 'Table Drop' ELSE chg_type end as chg_type, count(*) as recs
			FROM	MY_USHAREDB.UPGRADE_MANIFEST_LOAD
			WHERE 	chg_type in ('Table Add', 'View Added', 'Table Drop')
			GROUP BY 1,2) as mt
		ON mt.tablename = t1.tablename
		AND t1.databasename in ('MY_STAGE_DB')
		AND t1.tablekind IN ('T','V')
...
WHERE 	c.databasename = 'MY_EPICDB'
		...
		AND EXISTS (
			SELECT	1
			FROM	MY_USHAREDB.UPGRADE_DBC_TABLES t
			WHERE 	t.databasename IN ('MY_REPORT_DB')
...
WHERE	t0.databasename in ('MY_REPORT_DB')
		-- ------------------------------------------------------------
		-- only check for table ir the table is valid for the deployment
		-- ------------------------------------------------------------
		AND EXISTS (
...
,'IMG_ORD_VIEW'
		...
		AND NOT EXISTS (
			...
			WHERE	tr.databasename IN ('MY_REPORT_DB', 'MY_REPORT_DB')
...
WHERE	DTL.databasename IN ('MY_REPORT_DB')
